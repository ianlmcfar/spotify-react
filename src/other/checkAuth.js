...
function checkAuth (){
	...
	if (window.location.href.indexOf('access_token') === -1){
		if (!localStorage.authToken || moment().isAfter(moment.utc(JSON.parse(localStorage.authToken).expireTime)) ){
			window.location = `https://accounts.spotify.com/authorize/?client_id=${clientId}&response_type=token&redirect_uri=${redirect}&state=${clientState}?target=${targetLocation}`
		}
	}
	else{
		...
		targetLocation = decodeURIComponent(uri.substring(uri.indexOf('target=')+7))
